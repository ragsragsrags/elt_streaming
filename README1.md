...
If you see failed in replicate_testDB_messages test task, just trigger the whole process again it will be successful. The test task is created to validate task.  It can be commented out or in a separate dag for validation in say every 30 minutes so it does not interfere with the replication.
